...
function NotificationHandler() {
  ...
  useEffect(() => {
    const handleResponse = (response: Notifications.NotificationResponse) => {
      ...
      } else if (data?.activityId && ['chat', 'new_post', 'new_comment', 'rsvp_host', 'new_invite', 'limited_reopened', 'event_cancelled', 'event_deleted', 'host_ping', 'survey_ping'].includes(data.type ?? '')) {
        if (data.type === 'chat') {
          router.push(`/(app)/activity/${data.activityId}/chat?fromTab=chats`);
        } else if (data.type === 'new_post' || data.type === 'new_comment' || data.type === 'survey_ping') {
